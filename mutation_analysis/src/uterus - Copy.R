...
xx <- sort(table(uterus_mut_final$Hugo_Symbol),decreasing=TRUE)[1:10]
xx
#                                 

uterus_INPPL1 <- uterus_mut_final[which(uterus_mut_final$Hugo_Symbol == "INPPL1"), ]
uterus_no_INPPL1 <- uterus_mut_final[which(uterus_mut_final$Hugo_Symbol != "INPPL1"), ]

#find the most frequent variant type
calculate_mode(uterus_INPPL1$Variant_Classification)
#turns out to be Missense_Mutation

#create subset with INPPL1_Missense
uterus_INPPL1_Mis <- uterus_INPPL1[which(uterus_INPPL1$Variant_Classification == "Missense_Mutation"), ]
uterus_INPPL1_No_Mis <- uterus_INPPL1[which(uterus_INPPL1$Variant_Classification != "Missense_Mutation"), ]
uterus_INPPL1 <- droplevels(uterus_INPPL1)
#7 cell lines with INPPL1 Missense mutation

#subset of all ccl
uterus_ccl <- distinct(uterus_mutation, ccl, .keep_all= TRUE )

y <- length(uterus_INPPL1_Mis$ID)
x <- length(uterus$ccl)
x <- x - y


final_table <- data.frame(mutation = c("INPPL1", "other"), counts = c(y, x))

bar_title <- paste("Mut: INPPL1 Missense Mutation \n", y, "overlaps out of", x, "total")
...
uterus_ccl[match(uterus_INPPL1$ID, uterus_ccl$ID), ] <- uterus_INPPL1

uterus_ccl$Hugo_Symbol <- as.character(uterus_ccl$Hugo_Symbol)

uterus_ccl$Hugo_Symbol[uterus_ccl$Hugo_Symbol == "INPPL1"] <- "has Mut"
