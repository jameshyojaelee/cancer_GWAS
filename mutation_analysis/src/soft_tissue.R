...
calculate_mode(soft_tissue_mut_final$Hugo_Symbol)
#turns out to be PEG3!


soft_tissue_PEG3 <- soft_tissue_mut_final[which(soft_tissue_mut_final$Hugo_Symbol == "PEG3"), ]
soft_tissue_no_PEG3 <- soft_tissue_mut_final[which(soft_tissue_mut_final$Hugo_Symbol != "PEG3"), ]

#find the most frequent variant type
calculate_mode(soft_tissue_PEG3$Variant_Classification)
#turns out to be Missense_Mutation

#create subset with PEG3_Missense
soft_tissue_PEG3_Mis <- soft_tissue_PEG3[which(soft_tissue_PEG3$Variant_Classification == "Missense_Mutation"), ]
soft_tissue_PEG3_No_Mis <- soft_tissue_PEG3[which(soft_tissue_PEG3$Variant_Classification != "Missense_Mutation"), ]
soft_tissue_PEG3 <- droplevels(soft_tissue_PEG3)
#7 cell lines with PEG3 Missense mutation

#subset of all ccl
soft_tissue_ccl <- distinct(soft_tissue_mutation, ccl, .keep_all= TRUE )

y <- length(soft_tissue_PEG3_Mis$ID)
x <- length(soft_tissue$ccl)
x <- x - y


final_table <- data.frame(mutation = c("PEG3", "other"), counts = c(y, x))

bar_title <- paste("Mut: PEG3 Missense Mutation \n", y, "overlaps out of", x, "total")
...
soft_tissue_ccl[match(soft_tissue_PEG3$ID, soft_tissue_ccl$ID), ] <- soft_tissue_PEG3

soft_tissue_ccl$Hugo_Symbol <- as.character(soft_tissue_ccl$Hugo_Symbol)

soft_tissue_ccl$Hugo_Symbol[soft_tissue_ccl$Hugo_Symbol == "PEG3"] <- "has Mut"
