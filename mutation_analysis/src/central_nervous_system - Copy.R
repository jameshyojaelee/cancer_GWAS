...
xx <- sort(table(central_nervous_system_mut_final$Hugo_Symbol),decreasing=TRUE)[1:10]
xx
#                      

central_nervous_system_ETV1 <- central_nervous_system_mut_final[which(central_nervous_system_mut_final$Hugo_Symbol == "ETV1"), ]
central_nervous_system_no_ETV1 <- central_nervous_system_mut_final[which(central_nervous_system_mut_final$Hugo_Symbol != "ETV1"), ]

#find the most frequent variant type
calculate_mode(central_nervous_system_ETV1$Variant_Classification)
#turns out to be Missense_Mutation

#create subset with ETV1_Missense
central_nervous_system_ETV1_Mis <- central_nervous_system_ETV1[which(central_nervous_system_ETV1$Variant_Classification == "Missense_Mutation"), ]
central_nervous_system_ETV1_No_Mis <- central_nervous_system_ETV1[which(central_nervous_system_ETV1$Variant_Classification != "Missense_Mutation"), ]
central_nervous_system_ETV1 <- droplevels(central_nervous_system_ETV1)
#7 cell lines with ETV1 Missense mutation

#subset of all ccl
central_nervous_system_ccl <- distinct(central_nervous_system_mutation, ccl, .keep_all= TRUE )

y <- length(central_nervous_system_ETV1_Mis$ID)
x <- length(central_nervous_system$ccl)
x <- x - y


final_table <- data.frame(mutation = c("ETV1", "other"), counts = c(y, x))

bar_title <- paste("Mut: ETV1 Missense Mutation \n", y, "overlaps out of", x, "total")
...
central_nervous_system_ccl[match(central_nervous_system_ETV1$ID, central_nervous_system_ccl$ID), ] <- central_nervous_system_ETV1

central_nervous_system_ccl$Hugo_Symbol <- as.character(central_nervous_system_ccl$Hugo_Symbol)

central_nervous_system_ccl$Hugo_Symbol[central_nervous_system_ccl$Hugo_Symbol == "ETV1"] <- "has Mut"
