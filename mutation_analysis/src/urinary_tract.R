...
calculate_mode(urinary_tract_mut_final$Hugo_Symbol)
#turns out to be CHD6!


urinary_tract_CHD6 <- urinary_tract_mut_final[which(urinary_tract_mut_final$Hugo_Symbol == "CHD6"), ]
urinary_tract_no_CHD6 <- urinary_tract_mut_final[which(urinary_tract_mut_final$Hugo_Symbol != "CHD6"), ]

#find the most frequent variant type
calculate_mode(urinary_tract_CHD6$Variant_Classification)
#turns out to be Missense_Mutation

#create subset with CHD6_Missense
urinary_tract_CHD6_Mis <- urinary_tract_CHD6[which(urinary_tract_CHD6$Variant_Classification == "Missense_Mutation"), ]
urinary_tract_CHD6_No_Mis <- urinary_tract_CHD6[which(urinary_tract_CHD6$Variant_Classification != "Missense_Mutation"), ]
urinary_tract_CHD6 <- droplevels(urinary_tract_CHD6)
#7 cell lines with CHD6 Missense mutation

#subset of all ccl
urinary_tract_ccl <- distinct(urinary_tract_mutation, ccl, .keep_all= TRUE )

y <- length(urinary_tract_CHD6_Mis$ID)
x <- length(urinary_tract$ccl)
x <- x - y


final_table <- data.frame(mutation = c("CHD6", "other"), counts = c(y, x))

bar_title <- paste("Mut: CHD6 Missense Mutation \n", y, "overlaps out of", x, "total")
...
urinary_tract_ccl[match(urinary_tract_CHD6$ID, urinary_tract_ccl$ID), ] <- urinary_tract_CHD6

urinary_tract_ccl$Hugo_Symbol <- as.character(urinary_tract_ccl$Hugo_Symbol)

urinary_tract_ccl$Hugo_Symbol[urinary_tract_ccl$Hugo_Symbol == "CHD6"] <- "has Mut"
