...
xx <- sort(table(peripheral_nervous_system_mut_final$Hugo_Symbol),decreasing=TRUE)[1:10]
xx
#                                     


peripheral_nervous_system_PCNT <- peripheral_nervous_system_mut_final[which(peripheral_nervous_system_mut_final$Hugo_Symbol == "PCNT"), ]
peripheral_nervous_system_no_PCNT <- peripheral_nervous_system_mut_final[which(peripheral_nervous_system_mut_final$Hugo_Symbol != "PCNT"), ]

#find the most frequent variant type
calculate_mode(peripheral_nervous_system_PCNT$Variant_Classification)
#turns out to be Missense_Mutation

#create subset with PCNT_Missense
peripheral_nervous_system_PCNT_Mis <- peripheral_nervous_system_PCNT[which(peripheral_nervous_system_PCNT$Variant_Classification == "Missense_Mutation"), ]
peripheral_nervous_system_PCNT_No_Mis <- peripheral_nervous_system_PCNT[which(peripheral_nervous_system_PCNT$Variant_Classification != "Missense_Mutation"), ]
peripheral_nervous_system_PCNT <- droplevels(peripheral_nervous_system_PCNT)
#7 cell lines with PCNT Missense mutation

#subset of all ccl
peripheral_nervous_system_ccl <- distinct(peripheral_nervous_system_mutation, ccl, .keep_all= TRUE )

y <- length(peripheral_nervous_system_PCNT_Mis$ID)
x <- length(peripheral_nervous_system$ccl)
x <- x - y


final_table <- data.frame(mutation = c("PCNT", "other"), counts = c(y, x))

bar_title <- paste("Mut: PCNT Missense Mutation \n", y, "overlaps out of", x, "total")
...
peripheral_nervous_system_ccl[match(peripheral_nervous_system_PCNT$ID, peripheral_nervous_system_ccl$ID), ] <- peripheral_nervous_system_PCNT

peripheral_nervous_system_ccl$Hugo_Symbol <- as.character(peripheral_nervous_system_ccl$Hugo_Symbol)

peripheral_nervous_system_ccl$Hugo_Symbol[peripheral_nervous_system_ccl$Hugo_Symbol == "PCNT"] <- "has Mut"
