...
xx <- sort(table(cervix_mut_final$Hugo_Symbol),decreasing=TRUE)[1:10]
xx
#                           

cervix_INSR <- cervix_mut_final[which(cervix_mut_final$Hugo_Symbol == "INSR"), ]
cervix_no_INSR <- cervix_mut_final[which(cervix_mut_final$Hugo_Symbol != "INSR"), ]

#find the most frequent variant type
calculate_mode(cervix_INSR$Variant_Classification)
#turns out to be Missense_Mutation

#create subset with INSR_Missense
cervix_INSR_Mis <- cervix_INSR[which(cervix_INSR$Variant_Classification == "Missense_Mutation"), ]
cervix_INSR_No_Mis <- cervix_INSR[which(cervix_INSR$Variant_Classification != "Missense_Mutation"), ]
cervix_INSR <- droplevels(cervix_INSR)
#7 cell lines with INSR Missense mutation

#subset of all ccl
cervix_ccl <- distinct(cervix_mutation, ccl, .keep_all= TRUE )

y <- length(cervix_INSR_Mis$ID)
x <- length(cervix$ccl)
x <- x - y


final_table <- data.frame(mutation = c("INSR", "other"), counts = c(y, x))

bar_title <- paste("Mut: INSR Missense Mutation \n", y, "overlaps out of", x, "total")
...
cervix_ccl[match(cervix_INSR$ID, cervix_ccl$ID), ] <- cervix_INSR

cervix_ccl$Hugo_Symbol <- as.character(cervix_ccl$Hugo_Symbol)

cervix_ccl$Hugo_Symbol[cervix_ccl$Hugo_Symbol == "INSR"] <- "has Mut"
