...
calculate_mode(lung_mut_final$Hugo_Symbol)
#turns out to be ZKSCAN1!


lung_ZKSCAN1 <- lung_mut_final[which(lung_mut_final$Hugo_Symbol == "ZKSCAN1"), ]
lung_no_ZKSCAN1 <- lung_mut_final[which(lung_mut_final$Hugo_Symbol != "ZKSCAN1"), ]

#find the most frequent variant type
calculate_mode(lung_ZKSCAN1$Variant_Classification)
#turns out to be Missense_Mutation

#create subset with ZKSCAN1_Missense
lung_ZKSCAN1_Mis <- lung_ZKSCAN1[which(lung_ZKSCAN1$Variant_Classification == "Missense_Mutation"), ]
lung_ZKSCAN1_No_Mis <- lung_ZKSCAN1[which(lung_ZKSCAN1$Variant_Classification != "Missense_Mutation"), ]
lung_ZKSCAN1 <- droplevels(lung_ZKSCAN1)
#7 cell lines with ZKSCAN1 Missense mutation

#subset of all ccl
lung_ccl <- distinct(lung_mutation, ccl, .keep_all= TRUE )

y <- length(lung_ZKSCAN1_Mis$ID)
x <- length(lung$ccl)
x <- x - y


final_table <- data.frame(mutation = c("ZKSCAN1", "other"), counts = c(y, x))

bar_title <- paste("Mut: ZKSCAN1 Missense Mutation \n", y, "overlaps out of", x, "total")
...
lung_ccl[match(lung_ZKSCAN1$ID, lung_ccl$ID), ] <- lung_ZKSCAN1

lung_ccl$Hugo_Symbol <- as.character(lung_ccl$Hugo_Symbol)

lung_ccl$Hugo_Symbol[lung_ccl$Hugo_Symbol == "ZKSCAN1"] <- "has Mut"
