...
calculate_mode(kidney_mut_final$Hugo_Symbol)
#turns out to be PTEN!


kidney_PTEN <- kidney_mut_final[which(kidney_mut_final$Hugo_Symbol == "PTEN"), ]
kidney_no_PTEN <- kidney_mut_final[which(kidney_mut_final$Hugo_Symbol != "PTEN"), ]

#find the most frequent variant type
calculate_mode(kidney_PTEN$Variant_Classification)
#turns out to be Nonsense_Mutation

#create subset with PTEN_Missense
kidney_PTEN_Mis <- kidney_PTEN[which(kidney_PTEN$Variant_Classification == "Nonsense_Mutation"), ]
kidney_PTEN_No_Mis <- kidney_PTEN[which(kidney_PTEN$Variant_Classification != "Nonsense_Mutation"), ]
kidney_PTEN <- droplevels(kidney_PTEN)
#7 cell lines with PTEN Nonsense_Mutation

#subset of all ccl
kidney_ccl <- distinct(kidney_mutation, ccl, .keep_all= TRUE )

y <- length(kidney_PTEN_Mis$ID)
x <- length(kidney$ccl)
x <- x - y


final_table <- data.frame(mutation = c("PTEN", "other"), counts = c(y, x))

bar_title <- paste("Mut: PTEN Nonsense_Mutation \n", y, "overlaps out of", x, "total")
...
kidney_ccl[match(kidney_PTEN$ID, kidney_ccl$ID), ] <- kidney_PTEN

kidney_ccl$Hugo_Symbol <- as.character(kidney_ccl$Hugo_Symbol)

kidney_ccl$Hugo_Symbol[kidney_ccl$Hugo_Symbol == "PTEN"] <- "has Mut"
