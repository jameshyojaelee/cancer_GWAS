...
calculate_mode(gastric_mut_final$Hugo_Symbol)
#turns out to be C5orf42!


gastric_C5orf42 <- gastric_mut_final[which(gastric_mut_final$Hugo_Symbol == "C5orf42"), ]
gastric_no_C5orf42 <- gastric_mut_final[which(gastric_mut_final$Hugo_Symbol != "C5orf42"), ]

#find the most frequent variant type
calculate_mode(gastric_C5orf42$Variant_Classification)
#turns out to be Missense_Mutation

#create subset with C5orf42_Missense
gastric_C5orf42_Mis <- gastric_C5orf42[which(gastric_C5orf42$Variant_Classification == "Missense_Mutation"), ]
gastric_C5orf42_No_Mis <- gastric_C5orf42[which(gastric_C5orf42$Variant_Classification != "Missense_Mutation"), ]
gastric_C5orf42 <- droplevels(gastric_C5orf42)
#7 cell lines with C5orf42 Missense mutation

#subset of all ccl
gastric_ccl <- distinct(gastric_mutation, ccl, .keep_all= TRUE )

y <- length(gastric_C5orf42_Mis$ID)
x <- length(gastric$ccl)
x <- x - y


final_table <- data.frame(mutation = c("C5orf42", "other"), counts = c(y, x))

bar_title <- paste("Mut: C5orf42 Missense Mutation \n", y, "overlaps out of", x, "total")
...
gastric_ccl[match(gastric_C5orf42$ID, gastric_ccl$ID), ] <- gastric_C5orf42

gastric_ccl$Hugo_Symbol <- as.character(gastric_ccl$Hugo_Symbol)

gastric_ccl$Hugo_Symbol[gastric_ccl$Hugo_Symbol == "C5orf42"] <- "has Mut"
