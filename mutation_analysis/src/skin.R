...
calculate_mode(skin_mut_final$Hugo_Symbol)
#turns out to be MYO15A!


skin_MYO15A <- skin_mut_final[which(skin_mut_final$Hugo_Symbol == "MYO15A"), ]
skin_no_MYO15A <- skin_mut_final[which(skin_mut_final$Hugo_Symbol != "MYO15A"), ]

#find the most frequent variant type
calculate_mode(skin_MYO15A$Variant_Classification)
#turns out to be Missense_Mutation

#create subset with MYO15A_Missense
skin_MYO15A_Mis <- skin_MYO15A[which(skin_MYO15A$Variant_Classification == "Missense_Mutation"), ]
skin_MYO15A_No_Mis <- skin_MYO15A[which(skin_MYO15A$Variant_Classification != "Missense_Mutation"), ]
skin_MYO15A <- droplevels(skin_MYO15A)
#7 cell lines with MYO15A Missense mutation

#subset of all ccl
skin_ccl <- distinct(skin_mutation, ccl, .keep_all= TRUE )

y <- length(skin_MYO15A_Mis$ID)
x <- length(skin$ccl)
x <- x - y


final_table <- data.frame(mutation = c("MYO15A", "other"), counts = c(y, x))

bar_title <- paste("Mut: MYO15A Missense Mutation \n", y, "overlaps out of", x, "total")
...
skin_ccl[match(skin_MYO15A$ID, skin_ccl$ID), ] <- skin_MYO15A

skin_ccl$Hugo_Symbol <- as.character(skin_ccl$Hugo_Symbol)

skin_ccl$Hugo_Symbol[skin_ccl$Hugo_Symbol == "MYO15A"] <- "has Mut"
