...
calculate_mode(lymphocyte_mut_final$Hugo_Symbol)
#turns out to be EP300!


lymphocyte_EP300 <- lymphocyte_mut_final[which(lymphocyte_mut_final$Hugo_Symbol == "EP300"), ]
lymphocyte_no_EP300 <- lymphocyte_mut_final[which(lymphocyte_mut_final$Hugo_Symbol != "EP300"), ]

#find the most frequent variant type
calculate_mode(lymphocyte_EP300$Variant_Classification)
#turns out to be Missense_Mutation

#create subset with EP300_Missense
lymphocyte_EP300_Mis <- lymphocyte_EP300[which(lymphocyte_EP300$Variant_Classification == "Missense_Mutation"), ]
lymphocyte_EP300_No_Mis <- lymphocyte_EP300[which(lymphocyte_EP300$Variant_Classification != "Missense_Mutation"), ]
lymphocyte_EP300 <- droplevels(lymphocyte_EP300)
#7 cell lines with EP300 Missense mutation

#subset of all ccl
lymphocyte_ccl <- distinct(lymphocyte_mutation, ccl, .keep_all= TRUE )

y <- length(lymphocyte_EP300_Mis$ID)
x <- length(lymphocyte$ccl)
x <- x - y


final_table <- data.frame(mutation = c("EP300", "other"), counts = c(y, x))

bar_title <- paste("Mut: EP300 Missense Mutation \n", y, "overlaps out of", x, "total")
...
lymphocyte_ccl[match(lymphocyte_EP300$ID, lymphocyte_ccl$ID), ] <- lymphocyte_EP300

lymphocyte_ccl$Hugo_Symbol <- as.character(lymphocyte_ccl$Hugo_Symbol)

lymphocyte_ccl$Hugo_Symbol[lymphocyte_ccl$Hugo_Symbol == "EP300"] <- "has Mut"
