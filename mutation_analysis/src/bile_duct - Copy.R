...
xx <- sort(table(bile_duct_mut_final$Hugo_Symbol),decreasing=TRUE)[1:10]
xx
#                

bile_duct_ANK1 <- bile_duct_mut_final[which(bile_duct_mut_final$Hugo_Symbol == "ANK1"), ]
bile_duct_no_ANK1 <- bile_duct_mut_final[which(bile_duct_mut_final$Hugo_Symbol != "ANK1"), ]

#find the most frequent variant type
calculate_mode(bile_duct_ANK1$Variant_Classification)
#turns out to be Missense_Mutation

#create subset with ANK1_Missense
bile_duct_ANK1_Mis <- bile_duct_ANK1[which(bile_duct_ANK1$Variant_Classification == "Missense_Mutation"), ]
bile_duct_ANK1_No_Mis <- bile_duct_ANK1[which(bile_duct_ANK1$Variant_Classification != "Missense_Mutation"), ]
bile_duct_ANK1 <- droplevels(bile_duct_ANK1)
#7 cell lines with ANK1 Missense_Mutation

#subset of all ccl
bile_duct_ccl <- distinct(bile_duct_mutation, ccl, .keep_all= TRUE )

y <- length(bile_duct_ANK1_Mis$ID)
x <- length(bile_duct$ccl)
x <- x - y


final_table <- data.frame(mutation = c("ANK1", "other"), counts = c(y, x))

bar_title <- paste("Mut: ANK1 Missense_Mutation \n", y, "overlaps out of", x, "total")
...
bile_duct_ccl[match(bile_duct_ANK1$ID, bile_duct_ccl$ID), ] <- bile_duct_ANK1

bile_duct_ccl$Hugo_Symbol <- as.character(bile_duct_ccl$Hugo_Symbol)

bile_duct_ccl$Hugo_Symbol[bile_duct_ccl$Hugo_Symbol == "ANK1"] <- "has Mut"
