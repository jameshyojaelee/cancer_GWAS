...
calculate_mode(thyroid_mut_final$Hugo_Symbol)
#turns out to be SDK1!


thyroid_SDK1 <- thyroid_mut_final[which(thyroid_mut_final$Hugo_Symbol == "SDK1"), ]
thyroid_no_SDK1 <- thyroid_mut_final[which(thyroid_mut_final$Hugo_Symbol != "SDK1"), ]

#find the most frequent variant type
calculate_mode(thyroid_SDK1$Variant_Classification)
#turns out to be Missense_Mutation

#create subset with SDK1_Missense
thyroid_SDK1_Mis <- thyroid_SDK1[which(thyroid_SDK1$Variant_Classification == "Missense_Mutation"), ]
thyroid_SDK1_No_Mis <- thyroid_SDK1[which(thyroid_SDK1$Variant_Classification != "Missense_Mutation"), ]
thyroid_SDK1 <- droplevels(thyroid_SDK1)
#7 cell lines with SDK1 Missense mutation

#subset of all ccl
thyroid_ccl <- distinct(thyroid_mutation, ccl, .keep_all= TRUE )

y <- length(thyroid_SDK1_Mis$ID)
x <- length(thyroid$ccl)
x <- x - y


final_table <- data.frame(mutation = c("SDK1", "other"), counts = c(y, x))

bar_title <- paste("Mut: SDK1 Missense Mutation \n", y, "overlaps out of", x, "total")
...
thyroid_ccl[match(thyroid_SDK1$ID, thyroid_ccl$ID), ] <- thyroid_SDK1

thyroid_ccl$Hugo_Symbol <- as.character(thyroid_ccl$Hugo_Symbol)

thyroid_ccl$Hugo_Symbol[thyroid_ccl$Hugo_Symbol == "SDK1"] <- "has Mut"
