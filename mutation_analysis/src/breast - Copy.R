...
xx <- sort(table(breast_mut_final$Hugo_Symbol),decreasing=TRUE)[1:300]
xx
#                  

breast_ADAMTS12 <- breast_mut_final[which(breast_mut_final$Hugo_Symbol == "ADAMTS12"), ]
breast_no_ADAMTS12 <- breast_mut_final[which(breast_mut_final$Hugo_Symbol != "ADAMTS12"), ]

#find the most frequent variant type
calculate_mode(breast_ADAMTS12$Variant_Classification)
#turns out to be Missense_Mutation

#create subset with ADAMTS12_Missense
breast_ADAMTS12_Mis <- breast_ADAMTS12[which(breast_ADAMTS12$Variant_Classification == "Missense_Mutation"), ]
breast_ADAMTS12_No_Mis <- breast_ADAMTS12[which(breast_ADAMTS12$Variant_Classification != "Missense_Mutation"), ]
breast_ADAMTS12 <- droplevels(breast_ADAMTS12)
#7 cell lines with ADAMTS12 Missense mutation

#subset of all ccl
breast_ccl <- distinct(breast_mutation, ccl, .keep_all= TRUE )

y <- length(breast_ADAMTS12_Mis$ID)
x <- length(breast$ccl)
x <- x - y


final_table <- data.frame(mutation = c("ADAMTS12", "other"), counts = c(y, x))

bar_title <- paste("Mut: ADAMTS12 Missense Mutation \n", y, "overlaps out of", x, "total")
...
breast_ccl[match(breast_ADAMTS12$ID, breast_ccl$ID), ] <- breast_ADAMTS12

breast_ccl$Hugo_Symbol <- as.character(breast_ccl$Hugo_Symbol)

breast_ccl$Hugo_Symbol[breast_ccl$Hugo_Symbol == "ADAMTS12"] <- "has Mut"
