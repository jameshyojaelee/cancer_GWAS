...
calculate_mode(pancreas_mut_final$Hugo_Symbol)
#turns out to be SCN10A!


pancreas_SCN10A <- pancreas_mut_final[which(pancreas_mut_final$Hugo_Symbol == "SCN10A"), ]
pancreas_no_SCN10A <- pancreas_mut_final[which(pancreas_mut_final$Hugo_Symbol != "SCN10A"), ]

#find the most frequent variant type
calculate_mode(pancreas_SCN10A$Variant_Classification)
#turns out to be Missense_Mutation

#create subset with SCN10A_Missense
pancreas_SCN10A_Mis <- pancreas_SCN10A[which(pancreas_SCN10A$Variant_Classification == "Missense_Mutation"), ]
pancreas_SCN10A_No_Mis <- pancreas_SCN10A[which(pancreas_SCN10A$Variant_Classification != "Missense_Mutation"), ]
pancreas_SCN10A <- droplevels(pancreas_SCN10A)
#7 cell lines with SCN10A Missense mutation

#subset of all ccl
pancreas_ccl <- distinct(pancreas_mutation, ccl, .keep_all= TRUE )

y <- length(pancreas_SCN10A_Mis$ID)
x <- length(pancreas$ccl)
x <- x - y


final_table <- data.frame(mutation = c("SCN10A", "other"), counts = c(y, x))

bar_title <- paste("Mut: SCN10A Missense Mutation \n", y, "overlaps out of", x, "total")
...
pancreas_ccl[match(pancreas_SCN10A$ID, pancreas_ccl$ID), ] <- pancreas_SCN10A

pancreas_ccl$Hugo_Symbol <- as.character(pancreas_ccl$Hugo_Symbol)

pancreas_ccl$Hugo_Symbol[pancreas_ccl$Hugo_Symbol == "SCN10A"] <- "has Mut"
