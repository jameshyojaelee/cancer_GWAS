...
calculate_mode(plasma_cell_mut_final$Hugo_Symbol)
#turns out to be OBSCN!


plasma_cell_OBSCN <- plasma_cell_mut_final[which(plasma_cell_mut_final$Hugo_Symbol == "OBSCN"), ]
plasma_cell_no_OBSCN <- plasma_cell_mut_final[which(plasma_cell_mut_final$Hugo_Symbol != "OBSCN"), ]

#find the most frequent variant type
calculate_mode(plasma_cell_OBSCN$Variant_Classification)
#turns out to be Missense_Mutation

#create subset with OBSCN_Missense
plasma_cell_OBSCN_Mis <- plasma_cell_OBSCN[which(plasma_cell_OBSCN$Variant_Classification == "Missense_Mutation"), ]
plasma_cell_OBSCN_No_Mis <- plasma_cell_OBSCN[which(plasma_cell_OBSCN$Variant_Classification != "Missense_Mutation"), ]
plasma_cell_OBSCN <- droplevels(plasma_cell_OBSCN)
#7 cell lines with OBSCN Missense mutation

#subset of all ccl
plasma_cell_ccl <- distinct(plasma_cell_mutation, ccl, .keep_all= TRUE )

y <- length(plasma_cell_OBSCN_Mis$ID)
x <- length(plasma_cell$ccl)
x <- x - y


final_table <- data.frame(mutation = c("OBSCN", "other"), counts = c(y, x))

bar_title <- paste("Mut: OBSCN Missense Mutation \n", y, "overlaps out of", x, "total")
...
plasma_cell_ccl[match(plasma_cell_OBSCN$ID, plasma_cell_ccl$ID), ] <- plasma_cell_OBSCN

plasma_cell_ccl$Hugo_Symbol <- as.character(plasma_cell_ccl$Hugo_Symbol)

plasma_cell_ccl$Hugo_Symbol[plasma_cell_ccl$Hugo_Symbol == "OBSCN"] <- "has Mut"
